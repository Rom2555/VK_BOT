...
Программа-бот, разработанная в рамках курсовой работы по курсу **«Профессиональная работа с Python»** (Netology).  
Подбирает кандидатов для знакомств на основе возраста, пола и города, позволяет сохранять избранных, использовать чёрный список и выбирать интересы.
...
- **Избранное и чёрный список**  
  Пользователь может добавлять кандидатов в избранное или чёрный список, просматривать избранных и удалять их.

- **Выбор интересов**  
  Доступны категории: музыка, книги, спорт, путешествия, кино, игры. Можно отменить выбор в любой момент.

- **Без дублей**  
  Не показывает кандидатов, уже добавленных в избранное или чёрный список.
...
- `database/manager.py` — менеджер базы данных (работа с пользователями, кандидатами, избранным)  
- `database/models.py` — модели SQLAlchemy (таблицы: users, candidates, favorites)  
- `config.py` — загрузка переменных окружения и сборка DATABASE_URL  
